...
export default class GridItem extends React.Component<Props, State> {
	 ...
	 */
	calcWidth(w: number, colWidth: number): number {
		return Math.round(colWidth * w + Math.max(0, w - 1) * this.props.margin[0]);
	}
	...
	calcPosition(x: number, y: number, w: number, h: number, state: ?Object): Position {
		...
		const out = {
			...
			width: w === Infinity ? w : this.calcWidth(w, colWidth),
			height: h === Infinity ? h : this.calcHeight(h)
		};
		...
		return (e: Event, { node, deltaX, deltaY }: ReactDraggableCallbackData) => {
			...
			switch (handlerName) {
				...
				case 'onDrag': {
					...
					const { isBounded, w, h, containerWidth } = this.props;

					if (isBounded) {
						const { offsetParent } = node;

						if (offsetParent) {
							const bottomBoundary = offsetParent.clientHeight - this.calcHeight(h);
							if (top > bottomBoundary) top = bottomBoundary;
							if (top < 0) top = 0;

							const rightBoundary = containerWidth - this.calcWidth(w, this.calcColWidth());
